...
const RichTextEditor: React.FC<RichTextEditorProps> = ({
    ...
    setRows((prev) =>
      prev.map((row) => ({
        ...
        columns: row.columns.map((col) =>
          col.id === colId ? { ...col, content: html } : col,
        ),
      })),
    );
  };
  ...
  const insertProduct = (product: any) => {
    // Generate an HTML snippet representing the embedded product
    const html = `
      <div contenteditable="false" class="product-embed flex flex-col sm:flex-row items-center sm:items-start p-4 border border-spirit-200 rounded-xl bg-spirit-50 shadow-sm my-4 gap-4" data-product-id="${product.id}">
        <img src="${product.image}" class="w-24 h-24 object-cover rounded-lg flex-shrink-0" alt="${product.name}" />
        <div class="flex-1 text-center sm:text-left">
          <h4 class="font-bold text-spirit-900">${product.name}</h4>
          <p class="text-sm text-gray-500 font-medium py-1">${product.price}</p>
          <div class="mt-2 text-xs bg-spirit-900 text-white px-3 py-1.5 rounded-md inline-block font-bold shrink-0">View Product</div>
        </div>
      </div>
      <div><br/></div>
    `;
    ...
    if (mediaTab === "url") {
      ...
      if (mediaType === "image") {
        ...
        if (mediaUrl.includes("youtube.com") || mediaUrl.includes("youtu.be")) {
          const videoId =
            mediaUrl.split("v=")[1]?.split("&")[0] || mediaUrl.split("/").pop();
    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm relative">
      ...
      <div className="bg-gray-50 border-b border-gray-200 p-2 flex flex-wrap gap-2 items-center sticky top-0 z-20">
        ...
        <div className="flex bg-white rounded border border-gray-200 shadow-sm">
          <button
            type="button"
            onMouseDown={(e) => onToolbarBtnClick(e, "bold")}
          ...
          <button
            type="button"
            onMouseDown={(e) => onToolbarBtnClick(e, "italic")}
            className="p-2 hover:bg-gray-100 text-gray-700"
            title="Italic"
          >
            <Italic size={16} />
          ...
          <button
            type="button"
            onMouseDown={(e) => onToolbarBtnClick(e, "underline")}
            className="p-2 hover:bg-gray-100 text-gray-700"
            title="Underline"
          >
            <Underline size={16} />
        ...
        <div className="flex bg-white rounded border border-gray-200 shadow-sm">
          <button
            type="button"
            onMouseDown={(e) => onToolbarBtnClick(e, "justifyLeft")}
            className="p-2 hover:bg-gray-100 text-gray-700"
          >
            <AlignLeft size={16} />
          </button>
          <button
            type="button"
            onMouseDown={(e) => onToolbarBtnClick(e, "justifyCenter")}
            className="p-2 hover:bg-gray-100 text-gray-700"
          >
            <AlignCenter size={16} />
          </button>
          <button
            type="button"
            onMouseDown={(e) => onToolbarBtnClick(e, "justifyRight")}
            className="p-2 hover:bg-gray-100 text-gray-700"
          >
            <AlignRight size={16} />
          </button>
        ...
        <div className="flex items-center gap-2">
          <div
            className="flex bg-white rounded border border-gray-200 items-center px-2 py-1 shadow-sm"
            title="Text Color"
          >
      ...
      <div className="p-8 bg-gray-100 min-h-[500px] space-y-8">
        {rows.map((row, rIndex) => (
          <div
            key={row.id}
            className="relative group bg-white rounded-xl border border-dashed border-gray-300 hover:border-spirit-400 transition-colors shadow-sm overflow-hidden"
          >
            {/* Row Header / Controls */}
            <div className="bg-gray-50 border-b border-gray-200 px-4 py-2 flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center">
                  <button
                    type="button"
                    onClick={() => moveRowUp(rIndex)}
                    disabled={rIndex === 0}
                    className="p-1 text-gray-400 hover:text-spirit-600 disabled:opacity-30 transition-colors"
                    title="Move Up"
                  >
                    <ArrowUp size={16} />
                  </button>
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400 mb-2 px-1">
                    <span className="flex items-center">
                      <MoveHorizontal size={10} className="mr-1" /> Width %
                    </span>
                    <ColumnWidthInput
                      width={col.width}
      ...
      <div className="bg-gray-50 px-4 py-2 text-xs text-gray-500 border-t border-gray-200 flex justify-between">
        <span>
          Tip: Highlight text to format. Click 'Image' or 'Video' to insert
          media.
        </span>
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-6 w-96 animate-fade-in">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-gray-800 flex items-center">
                <Layout size={18} className="mr-2 text-spirit-600" /> Select
                Columns
              </h3>
              <button
                type="button"
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                type="button"
                onClick={() => addRow(1)}
                className="border border-gray-200 rounded-lg p-4 hover:bg-spirit-50 hover:border-spirit-300 transition flex flex-col items-center gap-2 group"
              >
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-6 w-96 animate-fade-in">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-gray-800 flex items-center capitalize">
                {mediaType === "image" ? (
                  <ImageIcon size={18} className="mr-2 text-spirit-600" />
                ) : (
                  <Video size={18} className="mr-2 text-spirit-600" />
                )}
                Insert {mediaType}
              </h3>
              <button
      ...
      {/* --- POPUP: PRODUCT INSERTION --- */}
      {showProductModal && (
        <div className="absolute inset-0 z-50 flex items-start justify-center pt-20 bg-black/20 backdrop-blur-[1px]">
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-6 w-[500px] animate-fade-in max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-center mb-4 shrink-0">
              <h3 className="font-bold text-gray-800 flex items-center">
                <ShoppingBag size={18} className="mr-2 text-spirit-600" />
                Select Product
              </h3>
              <button
                type="button"
              ...
              {products.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-sm text-gray-500">
                    No products available. Add some in the Products tab first.
                  </p>
                </div>
              ) : (
                products.map((product) => (
                  <div
                    ...
                    className="flex items-center gap-4 p-3 border border-gray-200 rounded-xl hover:border-spirit-400 hover:bg-spirit-50 cursor-pointer transition-colors"
                  >
                    <img
                      src={product.image}
                      alt={product.name}
